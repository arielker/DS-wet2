...
public:
	int _key;
	T* _value;
	bool _isMainHash; ///this parameter determines whether or not to delete _value in the d'tor
...
public:
	explicit HashTable(bool isMainHash){
		...
		for (int i = 0; i < _currSize; i++)
			_hashTable[i] = nullptr; /// means that this cell hasn't been initialized (not deleted)
	}
	...
	void doubleTable(){
		...
		///dtor needs to delete all "tuples" of HashTableElement but not the servers, because "id tree" method
		for (int i = 0; i < oldSize; i++){
			if (oldhashTable[i] != nullptr){
	...
	void quarterTable(){
		...
		for (int i = 0; i<oldSize; i++){
			if (oldhashTable[i] != nullptr && oldhashTable[i]->_key != -1){
				insertHash(oldhashTable[i]->_key, oldhashTable[i]->_value);
			}
		}
		///dtor needs to delete all "tuples" of HashTableElement but not the servers, because "id tree" method
	...
	bool deleteFromHash(int key) {
		...
		while (_hashTable[index] != nullptr){
			if (_hashTable[index]->_key == key){
				...
				_hashTable[index]->_value = nullptr;
				_numOfExistingElements--;
